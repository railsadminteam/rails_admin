class RailsAdmin::History < ActiveRecord::Base
  ...
  def self.create_history_item(message, object, abstract_model, user) # rubocop:disable ParameterLists
    create(
  ...
  def self.history_for_model(abstract_model, query, sort, sort_reverse, all, page, per_page = (RailsAdmin::Config.default_items_per_page || 20)) # rubocop:disable ParameterLists
  ...
  def self.history_for_object(abstract_model, object, query, sort, sort_reverse, all, page, per_page = (RailsAdmin::Config.default_items_per_page || 20)) # rubocop:disable ParameterLists
...
protected
  ...
  def self.history_for_model_or_object(history, abstract_model, query, sort, sort_reverse, all, page, per_page) # rubocop:disable ParameterLists
