...
MLB::Team.all.each do |mlb_team|
  unless league_model.where(:name => mlb_team.league).first
    league = league_model.model.new(:name => mlb_team.league)
    league.save!
  end
  unless division_model.where(:name => mlb_team.division).first
    division = division_model.model.new(:name => mlb_team.division, :league => league)
    division.save!
  end
  unless team_model.where(:name => mlb_team.name).first
