...
describe 'RailsAdmin Config DSL List Section' do
  ...
  describe "items' fields" do
    ...
    it 'allows addition of virtual fields (object methods)' do
      ...
      visit index_path(:model_name => 'team')
      expect(find('tbody tr:nth-child(1) td:nth-child(4)')).to have_content(@players.map(&:name).join(', '))
    end
  end
  ...
  describe 'default sorting' do
    ...
    let(:players) do
      [{:name => 'Jackie Robinson',  :created_at => today,            :team_id => rand(999), :number => 42},
       {:name => 'Deibinson Romero', :created_at => (today - 2.days), :team_id => rand(999), :number => 13},
       {:name => 'Sandy Koufax',     :created_at => (today - 1.days), :team_id => rand(999), :number => 32}]
    end
    let(:leagues) do
