...
describe 'RailsAdmin::Adapters::ActiveRecord', active_record: true do
  ...
  describe '#build_statement' do
    ...
    it 'supports date type query' do
      scope = FieldTest.all
      expect(@abstract_model.send(:filter_scope, scope, 'date_field' => {'1' => {v: ['', '01 February 2012', '01 March 2012'], o: 'between'}}).where_values).to eq(["(field_tests.date_field BETWEEN '2012-02-01' AND '2012-03-01')"])
      expect(@abstract_model.send(:filter_scope, scope, 'date_field' => {'1' => {v: ['', '01 March 2012', ''], o: 'between'}}).where_values).to eq(["(field_tests.date_field >= '2012-03-01')"])
      expect(@abstract_model.send(:filter_scope, scope, 'date_field' => {'1' => {v: ['', '', '01 February 2012'], o: 'between'}}).where_values).to eq(["(field_tests.date_field <= '2012-02-01')"])
      expect(@abstract_model.send(:filter_scope, scope, 'date_field' => {'1' => {v: ['01 February 2012'], o: 'default'}}).where_values).to eq(["(field_tests.date_field BETWEEN '2012-02-01' AND '2012-02-01')"])
    end

    it 'supports datetime type query' do
      scope = FieldTest.all
      expect(@abstract_model.send(:filter_scope, scope,  'datetime_field' => {'1' => {v: ['', '01 February 2012', '01 March 2012'], o: 'between'}}).where_values).to eq(scope.where(['(field_tests.datetime_field BETWEEN ? AND ?)', Time.local(2012, 2, 1), Time.local(2012, 3, 1).end_of_day]).where_values)
      expect(@abstract_model.send(:filter_scope, scope, 'datetime_field' => {'1' => {v: ['', '01 March 2012', ''], o: 'between'}}).where_values).to eq(scope.where(['(field_tests.datetime_field >= ?)', Time.local(2012, 3, 1)]).where_values)
      expect(@abstract_model.send(:filter_scope, scope, 'datetime_field' => {'1' => {v: ['', '', '01 February 2012'], o: 'between'}}).where_values).to eq(scope.where(['(field_tests.datetime_field <= ?)', Time.local(2012, 2, 1).end_of_day]).where_values)
      expect(@abstract_model.send(:filter_scope, scope, 'datetime_field' => {'1' => {v: ['01 February 2012'], o: 'default'}}).where_values).to eq(scope.where(['(field_tests.datetime_field BETWEEN ? AND ?)', Time.local(2012, 2, 1), Time.local(2012, 2, 1).end_of_day]).where_values)
    end
