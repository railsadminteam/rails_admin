  ...
  class FormBuilder < ::ActionView::Helpers::FormBuilder
    ...
    def fieldset_for(fieldset, nested_in)
      ...
      @template.content_tag :fieldset do
        contents = []
        contents << @template.content_tag(:legend, %(<i class="icon ion-chevron-#{(fieldset.active? ? 'down' : 'right')}"></i> #{fieldset.label}).html_safe, style: "#{fieldset.name == :default ? 'display:none' : ''}")
