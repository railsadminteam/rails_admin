...
RailsAdmin ships a prebuilt bundle and serves it through your app's existing pipeline; `config.asset_source` (`:propshaft` / `:sprockets` / `:external` / a callable, auto-detected when unset) selects how. See [Asset delivery](asset-delivery.md) for the full picture.
