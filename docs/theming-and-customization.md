...
RailsAdmin exposes a handful of CSS custom properties on `body.rails_admin`. Override them from a stylesheet or a `_head_custom` `<style>` block (see below) to restyle the RailsAdmin chrome without recompiling its CSS.
...
RailsAdmin renders an empty `_head_custom` partial at the end of its `<head>`. Override it in your app to add your own `<script>` / `<style>` / `<link>` tags, no build step required:
...
This replaces the old `app/assets/{stylesheets,javascripts}/rails_admin/custom/*` override files, which only worked under Sprockets.

## Adding to the bundle (`:external` setup)

When you build the assets yourself (`config.asset_source = :external`), the installer generates `rails_admin.js` and `rails_admin.scss` (location varies depending on [the asset delivery method](base-configuration.md#asset-delivery)). You can freely add your custom code or import a new dependency there, including Bootstrap variable overrides around the `@import "rails_admin/src/rails_admin/styles/base.scss"` line as shown above.
