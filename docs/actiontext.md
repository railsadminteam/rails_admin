...
The bundle already includes the `@rails/actiontext` and `@rails/activestorage` glue, so attachment uploads work out of the box. Trix itself is loaded from a CDN when a `<trix-editor>` appears - to serve it statically, use `:external` below.
