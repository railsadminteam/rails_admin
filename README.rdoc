...
RailsAdmin provides it's out of the box administrative interface by inspecting application's model files and by following some Rails conventions. For a more tailored experience it also provides a configuration DSL which allows you to customize many aspects of the CRUD interface.
...
You can hide a model from the top navigation by marking it's <tt>visible</tt> option as false within model's navigation configuration section:
