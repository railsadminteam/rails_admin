...
RailsAdmin was conceived as a port of MerbAdmin[http://github.com/sferik/merb-admin] to Rails 3 and implemented as a {Ruby Summer of Code project}[http://www.rubysoc.org/projects] by Bogdan Gaza with mentors {Erik Michaels-Ober}[http://github.com/sferik], {Yehuda Katz}[http://github.com/wycats], {Rodrigo Rosenfeld Rosas}[http://github.com/rosenfeld], {Luke van der Hoeven}[http://github.com/plukevdh], and {Rein Henrichs}[http://github.com/reinh].
...
In your <tt>Gemfile</tt>, add the following dependency:
  gem 'rails_admin', :git => "git://github.com/sferik/merb-admin.git"
Run:
  $ bundle update
And then run:
  $ rails generate rails_admin:install_admin
This task will install RailsAdmin. During this process, it checks for Devise[http://github.com/plataformatec/devise] and installs it if you don't already have it installed. Devise[http://github.com/plataformatec/devise] is required for authentication to protect your data from anonymous users.
...
* and everyone who contributed to MerbAdmin[http://github.com/sferik/merb-admin] in the first place.
